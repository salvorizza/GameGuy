...
void gbz80_apu_clock(gbz80_apu_t* apu){
	...
	if (apu_power) {
		...
		if (apu->frame_sequencer.sweep_clock && apu->channel_1.sweeper.enabled) {
			...
			uint8_t sweep_state = gbz80_apu_frequency_sweep_update(&apu->channel_1.sweeper);
		...
		if (apu->frame_sequencer.volume_envelope_clock) {
		...
		if (apu->channel_4.base.frequency_timer.period != 0 && apu->channel_4.base.length_counter.enabled == 1 && gbz80_update_timer(&apu->channel_4.base.frequency_timer)) {
			...
			uint8_t xor_result = common_get8_bit(apu->channel_4.lfsr ^ (apu->channel_4.lfsr >> 1), 0);
			...
			uint8_t wave_output_duty = common_get8_bit((uint8_t)(~apu->channel_4.lfsr), 0);
...
uint8_t gbz80_apu_memory_write(gbz80_apu_t* apu, uint16_t address, uint8_t current_value, uint8_t* val) {
	...
	switch (address) {
		// NRx0
		case NRx0(1): {
			apu->channel_1.sweeper.shift = common_get8_bit_range(*val, 0, 2);
			apu->channel_1.sweeper.negate = common_get8_bit(*val, 3);
			uint8_t period = common_get8_bit_range(*val, 4, 6);
			if (period == 0) period = 8;
			gbz80_init_timer(&apu->channel_1.sweeper.timer, period);
			break;
		}
		...
		case NRx4(1): {
			uint8_t nr11 = gbz80_memory_read_internal(apu->instance, NRx1(1));
			gbz80_apu_length_enable_obscure_behavior(&apu->frame_sequencer, &apu->channel_1.base, 0, *val, current_value, &nr11, &nr52);
			gbz80_memory_write_internal(apu->instance, NRx1(1), nr11);
			gbz80_memory_write_internal(apu->instance, NR52, nr52);
		...
		case NRx4(2): {
			uint8_t nr21 = gbz80_memory_read_internal(apu->instance, NRx1(2));
			gbz80_apu_length_enable_obscure_behavior(&apu->frame_sequencer, &apu->channel_2.base, 1, *val, current_value, &nr21, &nr52);
			gbz80_memory_write_internal(apu->instance, NRx1(2), nr21);
			gbz80_memory_write_internal(apu->instance, NR52, nr52);
		...
		case NRx4(3): {
			uint8_t nr31 = gbz80_memory_read_internal(apu->instance, NRx1(3));
			gbz80_apu_length_enable_obscure_behavior(&apu->frame_sequencer, &apu->channel_3.base, 2, *val, current_value, &nr31, &nr52);
			gbz80_memory_write_internal(apu->instance, NRx1(3), nr31);
			gbz80_memory_write_internal(apu->instance, NR52, nr52);
		...
		case NRx4(4): {
			uint8_t nr41 = gbz80_memory_read_internal(apu->instance, NRx1(4));
			gbz80_apu_length_enable_obscure_behavior(&apu->frame_sequencer, &apu->channel_4.base, 3, *val, current_value, &nr41, &nr52);
			gbz80_memory_write_internal(apu->instance, NRx1(4), nr41);
			gbz80_memory_write_internal(apu->instance, NR52, nr52);
...
void gbz80_apu_trigger_channel1(gbz80_apu_t* apu) {
	...
	uint8_t length_enable = common_get8_bit(nr14, 6);
	if (length_counter_clock == 0 && length_enable && length_counter_period == 64 && apu->channel_1.base.length_counter.counter == 0) {
	...
	if (apu->channel_1.base.dac_enable == 0) {
		channel_enabled = 0;
	}
...
void gbz80_apu_trigger_channel2(gbz80_apu_t* apu) {
	...
	uint8_t nr24 = gbz80_memory_read_internal(apu->instance, NRx4(2));
	uint8_t nr52 = gbz80_memory_read_internal(apu->instance, NR52);
	uint8_t channel_enabled = apu->channel_2.base.dac_enable;
	...
	uint8_t length_enable = common_get8_bit(nr24, 6);
	if (length_counter_clock == 0 && length_enable && length_counter_period == 64 && apu->channel_2.base.length_counter.counter == 0) {
		length_counter_period = 63;
	}
...
void gbz80_apu_trigger_channel3(gbz80_apu_t* apu) {
	...
	uint8_t nr34 = gbz80_memory_read_internal(apu->instance, NRx4(3));
	uint8_t nr52 = gbz80_memory_read_internal(apu->instance, NR52);
	uint8_t channel_enabled = apu->channel_3.base.dac_enable;
	...
	uint8_t length_enable = common_get8_bit(nr34, 6);
	if (length_counter_clock == 0 && length_enable && length_counter_period == 256 && apu->channel_3.base.length_counter.counter == 0) {
		length_counter_period = 255;
	}
...
void gbz80_apu_trigger_channel4(gbz80_apu_t* apu) {
	...
	uint8_t nr44 = gbz80_memory_read_internal(apu->instance, NRx4(4));
	uint8_t nr52 = gbz80_memory_read_internal(apu->instance, NR52);
	uint8_t channel_enabled = apu->channel_4.base.dac_enable;
	...
	uint8_t length_enable = common_get8_bit(nr44, 6);
	if (length_counter_clock == 0 && length_enable && length_counter_period == 64 && apu->channel_4.base.length_counter.counter == 0) {
		length_counter_period = 63;
	}
...
uint16_t gbz80_apu_frequency_sweep_calc(gbz80_apu_frequency_sweep_t* frequency_sweep, uint8_t* channel_enabled) {
	uint16_t new_frequency = frequency_sweep->frequency_shadow >> frequency_sweep->shift;

	if (frequency_sweep->negate) {
...
uint8_t gbz80_apu_frequency_sweep_init(gbz80_apu_frequency_sweep_t* frequency_sweep, uint16_t frequency, uint8_t sweep_time, uint8_t decrease, uint8_t sweep_shift) {
	...
	frequency_sweep->negate = decrease;
	frequency_sweep->shift = sweep_shift;

	if (sweep_shift != 0) {
		uint16_t new_frequency = gbz80_apu_frequency_sweep_calc(frequency_sweep, &channel_enabled);
	}
...
uint8_t gbz80_apu_frequency_sweep_update(gbz80_apu_frequency_sweep_t* frequency_sweep) {
	...
	if (frequency_sweep->timer.period != 0 && gbz80_update_timer(&frequency_sweep->timer)) {
		...
		if (frequency_sweep->enabled && frequency_sweep->timer.period > 0) {
			...
			if (channel_enabled && frequency_sweep->shift > 0) {
...
{
	uint8_t length_enable = common_get8_bit(nrx4, 6);
	uint8_t prev_length_enable = common_get8_bit(prev_nrx4, 6);
	uint8_t length_counter_clock, sweep_clock, volume_envelope_clock;
	frame_sequencer_calc_clock(frame_sequencer->num_step, &length_counter_clock, &sweep_clock, &volume_envelope_clock);
	if (length_counter_clock == 0 && prev_length_enable == 0 && length_enable == 1 && channel->length_counter.counter != 0) {
		channel->length_counter.counter -= 1;
		if (channel->length_counter.counter == 0) {
			channel->length_counter.enabled = 0;

			if (channel_number == 2) {
				common_change8_bit_range(nrx1, 0, 7, 0);
			} else {
				common_change8_bit_range(nrx1, 0, 5, 0);
			}
			common_change8_bit(nr52, channel_number, 0);
		}
	}
}
