...
{
public:
	AudioManager(std::wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
	...
	bool Create(std::wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_thread = std::thread(&AudioManager::MainThread, this);
...
private:
	...
	{
		((AudioManager*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
